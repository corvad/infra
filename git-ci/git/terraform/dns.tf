...
# Creates a new remotely-managed tunnel for the auth server.
resource "cloudflare_zero_trust_tunnel_cloudflared" "auth_tunnel" {
  account_id = var.cloudflare_account_id
  name       = "Auth tunnel"
  config_src = "cloudflare"
}
...
# Reads the token used to run the tunnel on the server.
data "cloudflare_zero_trust_tunnel_cloudflared_token" "auth_tunnel_token" {
  account_id = var.cloudflare_account_id
  tunnel_id  = cloudflare_zero_trust_tunnel_cloudflared.auth_tunnel.id
}

# Creates the CNAME record that routes auth.${var.cloudflare_zone} to the tunnel.
resource "cloudflare_dns_record" "auth" {
  zone_id = var.cloudflare_zone_id
  name    = "auth"
  content = "${cloudflare_zero_trust_tunnel_cloudflared.auth_tunnel.id}.cfargotunnel.com"
  type    = "CNAME"
  ttl     = 1
  proxied = true
}
...
# Configures tunnel with a published application for clientless access.
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "auth_tunnel_config" {
  tunnel_id  = cloudflare_zero_trust_tunnel_cloudflared.auth_tunnel.id
  account_id = var.cloudflare_account_id
  config = {
    ingress = [
      {
        hostname = "auth.${var.cloudflare_zone}"
        service  = "http://localhost:8080"
      },
      {
        service = "http_status:404"
